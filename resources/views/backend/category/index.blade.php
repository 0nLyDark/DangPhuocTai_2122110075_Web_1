                  ...
                  <div class="row">
                    <div class="col-md-3">
                        <form action="{{ route('admin.category.store') }}" method="post" enctype="multipart/form-data" >
                          ...
                          <input type="text" id="name" name="name" placeholder="Nhập tên danh mục" class="form-control">
                          @error('name')
                            ...
                            @foreach ($list as $row)
                              <tr>
                                  ...
                                  <td style="width: 150px" class="text-center">
                                      <img src="{{ asset('image/categorys/'.$row->image) }}" alt="{{ $row->image }}" class="img-fluid">
                                  </td>
                                  <td>{{ $row->name }}</td>
                                  <td>{{ $row->slug }}</td>
                                  ...
                                  <td style="width: 180px" class="text-center ">
                                      <a data-url="{{ route('admin.category.status',$id) }}" name="status" data-id="{{ $row->id }}" class="btn btn-sm ">
                                          @if ($row->status==1)
                                      ...
                                      <a href="{{ route('admin.category.show',$id) }}" class="btn btn-sm ">
                                          <i class="far fa-eye text-info"></i>
                                      </a> 
                                      <a href="{{ route('admin.category.edit',$id) }}" class="btn btn-sm ">
                                          <i class="fas fa-edit text-primary"></i>
                                      </a>
                                      <a data-url="{{ route('admin.category.delete',$id) }}" name="delete" data-id="{{ $row->id }}" class="btn btn-sm ">
